...
export const GET: APIRoute = async ({ request }) => {
  ...
  const data = await fetch(`https://api.atlassian.com/ex/jira/${id}/rest/api/3/issuetype/project?projectId=${projectId}`, {
    headers: {
  ...
  const fieldValues = json.map(({ id, name }: {id: string, name: string}) => ({
    key: id,
    value: name,
  }));
